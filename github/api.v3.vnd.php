...
if ( $error ) {
	...
	<ul>
		...
		<li><code>media_type</code> (required, see
			<a href="http://developer.github.com/v3/media/">Media Types</a>)
		</li>
	</ul>
	<p>
	...
	<p>Here's an example that queries the <code>habari</code> repo, which is owned by the
		<code>habari</code> user, for all Issue Comments created since <code>2014-01-09T00:47+0000</code>, sorted by
		<code>created</code>, ordered by <code>desc</code>, with the comment body returned in HTML format:</p>
	<p>
	...
	<h2>Fork Me / Source Code</h2>
	<p>The source code for this script is available
		<a href="https://github.com/raamdev/tools.raam.org/blob/master/github/api.v3.vnd.php">on GitHub</a>.</p>
	<?php
	exit;
}

// Media Type may contain a plus (+) character, which we need to preserve for sending the header to GitHub
$media_type = urlencode( $_GET['media_type'] );
$media_type = str_replace( "+", "%2B", $media_type );
$media_type = urldecode( $media_type );
